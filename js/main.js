...
setInterval(function(){
    if(!localStorage.getItem("userLogged") && popup.style.display === "none"){
        popup.style.display = "flex";
    }
}, 30000);
...
emailjs.send("service_j08fdih","template_14467na",{
otp:generatedOTP,
email:email
})
.then(function(){
...
const phone = document.getElementById("userPhone").value;
const email = document.getElementById("userEmail").value;
const organization = document.getElementById("userOrg").value || "";

fetch("https://script.google.com/macros/s/AKfycbyDhUCzK0l01LqQ7jdiz6m-ZPZEn1oSscdJQKQIacmL7sa3NJJBqbVuhIMC0WezBT3p/exec", {
  method: "POST",
  mode: "no-cors",
  headers: {
...
function openMember(name, role, desc, email, avatar){
...
document.getElementById("memberEmail").innerText=email;
document.getElementById("memberEmail").href="mailto:"+email;
document.getElementById("memberAvatar").innerText=avatar;
